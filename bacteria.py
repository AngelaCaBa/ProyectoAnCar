...
class bacteria():
    ...
    def evalua_individuo(self, individuo, evaluador):
        score = 0
        for col in range(len(individuo[0])):
            columna = [seq[col] for seq in individuo]
            pares = self.obtener_pares_unicos(columna)
            for par in pares:
                score += evaluador.getScore(par[0], par[1])
        return score

    def refinamiento_local(self, individuo, evaluador):
        # Convertimos el individuo a lista de listas para poder modificarlo
        mejorado = [list(seq) for seq in individuo]

        for i in range(len(mejorado)):  # recorre cada secuencia
            j = 1
            while j < len(mejorado[i]) - 1:
                if mejorado[i][j] == "-":
                    # Mover gap a la izquierda
                    if mejorado[i][j - 1] != "-":
                        temp = mejorado[i][:j - 1] + ["-"] + [mejorado[i][j - 1]] + mejorado[i][j + 1:]
                        nuevo_individuo = [list(seq) for seq in mejorado]
                        nuevo_individuo[i] = temp
                        if self.evalua_individuo(nuevo_individuo, evaluador) > self.evalua_individuo(mejorado, evaluador):
                            mejorado = nuevo_individuo
                            continue
                    # Mover gap a la derecha
                    if mejorado[i][j + 1] != "-":
                        temp = mejorado[i][:j] + [mejorado[i][j + 1]] + ["-"] + mejorado[i][j + 2:]
                        nuevo_individuo = [list(seq) for seq in mejorado]
                        nuevo_individuo[i] = temp
                        if self.evalua_individuo(nuevo_individuo, evaluador) > self.evalua_individuo(mejorado, evaluador):
                            mejorado = nuevo_individuo
    ...
    def aplicar_refinamiento_mejor(self, veryBest):
        evaluador = evaluadorBlosum()
        refinado = self.refinamiento_local(veryBest[2], evaluador)
        score_refinado = self.evalua_individuo(refinado, evaluador)
        if score_refinado > veryBest[1]:
